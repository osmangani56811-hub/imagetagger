...
async function analyzeOne(file, genOptions){
  ...
  if (data && data.error){
    ...
    saveToHistory({ file: file.name, mode: genOptions.mode, ...data, date: new Date().toISOString() });
  }
...
async function redoOne(idx){
  ...
  if (data && data.error){
    ...
    saveToHistory({ file: originalFile.name, mode, ...data, date: new Date().toISOString() });
  }
  renderResults();
}
...
function showHistory(){
  const hist = JSON.parse(localStorage.getItem("imgtagger_history") || "[]");
  if (hist.length === 0){ alert("History is empty."); return; }
