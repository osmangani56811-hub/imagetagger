...
<div class="card">
  ...
  <div class="btnrow">
    ...
    <button class="btn-saveall" id="saveAllBtn">💾 Save All Into Images (ZIP)</button>
  </div>
...
function exportCSV(){
  ...
  a.download = "imagetagger_" + currentPlatform + "_export.csv";
  a.click();
}
...
async function saveAllMetadataToImages(){
  ...
  for (const r of usable){
    ...
    try{
      ...
      const outName = originalFile.name.replace(/\.jpe?g$/i, "") + "_tagged.jpg";
  ...
  const a = document.createElement("a");
  a.href = URL.createObjectURL(blob);
  a.download = "imagetagger_tagged_images.zip";
  a.click();
